...
class MultivariateNormalBase(nn.Module):
    ...
    Args:
        ...
        mean  (torch.Tensor): Mean of the distribution. If None, initialized to zeros. (Default: None)
        var   (torch.Tensor): Variance of the distribution. If None, initialized to ones. (Default: None)
    ...
    def log_prob(self, z_samples, embedded_strain=None):
        """
        Computes the log probability of the samples given the distribution.

        Args:
            z_samples (torch.Tensor): Samples of dim [Nbatch, self.dim] ie 1 sample per batch
            embedded_strain (torch.Tensor, unused): Embedded strain tensor. (Default: None)
    ...
    def sample(self, num_samples, embedded_strain=None):
        ...
        Returns:
            torch.Tensor: Samples of dim [num_samples, self.dim]
        """
...
class ConditionalMultivariateNormalBase(nn.Module):
    ...
    def log_prob(self, z_samples, embedded_strain):
        ...
        Args:
            z_samples (torch.Tensor): Samples of dim [Nbatch, self.dim] ie 1 sample per batch
            embedded_strain (torch.Tensor): Embedded strain tensor.
...
class ResampledMultivariateNormalBase(nn.Module):
    ...
    def log_prob(self, z_samples, embedded_strain=None):
        ...
        .. math::
            ...
            \log p(x) = \log \left( (1 - \alpha) \frac{p(x)}{Z} + \alpha \right)

        where :math:`\alpha = (1 - Z)^{T-1}` and :math:`Z` is the estimated partition function.
    ...
    def sample(self, num_samples, embedded_strain=None):
        """
        Samples from the prior distribution using the Learned Acceptance Rejection Sampling (LARS) procedure.
...
class ResampledConditionalMultivariateNormalBase(ResampledMultivariateNormalBase):
    ...
    def log_prob(self, z_samples, embedded_strain=None):
        r"""
        Computes the log probability of the samples given the distribution.
        Similar to the ResampledMultivariateNormalBase.log_prob but with the ConditionalMultivariateNormalBase
...
class MultivariateGaussianMixtureBase(nn.Module):
    ...
    .. math::
        ...
        p(x) = \sum_{i=1}^{N} w_i \mathcal{N}_i(x)

    where :math:`w_i` are the mixture weights and :math:`\mathcal{N}_i(x)` are the components of the mixture.
...
class VonMisesNormal(nn.Module):
    r"""
    Mixture of Multivariate Normal and VonMises distributions.

    The Multivairate Normal is defined as:

    .. math::

        p(x) = \frac{1}{\sqrt{(2\pi)^{d}|\Sigma|}} \exp\left(-\frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu)\right)

    The VonMises distribution is defined as:

    .. math::

        p(x) = \frac{1}{2\pi I_0(\kappa)} \exp(\kappa \cos(x))

    where :math:`I_0` is the modified Bessel function of the first kind of order 0. 

    The distribution is initialized to zero mean and unit variance for the Normal and zero mean and unit concentration for the VonMises.

    Args:
        parameters (dict) : Dictionary with the parameters of the distributions.
        dim (dict)        : Dictionary with the dimensions of the distributions.
        trainable (bool)  : Whether to train the parameters (means and stds) of the distribution during training. (Default: False)

    Note:
        This base prior might be useful for cases where the latent space is a mixture of bounded and periodic variables.
    """
    ...
    def log_prob(self, z_samples, embedded_strain=None):
        ...
        Args:
            z_samples (torch.Tensor): Samples of dim [Nbatch, self.Normal_dim+self.VonMises_dim] ie 1 sample per batch
            embedded_strain (torch.Tensor, unused): Embedded strain tensor. (Default: None)
    ...
    def sample(self, num_samples, embedded_strain=None):
        ...
        Returns:
            torch.Tensor: Samples of dim [num_samples, self.Normal_dim+self.VonMises_dim]
        """
