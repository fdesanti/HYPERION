...
class StrainResNet(nn.Module):
    ...
    def __init__(self, 
        ...
        super(StrainResNet, self).__init__()
        ...
        self.CNN = nn.Sequential(
                 ...
                 #nn.Conv1d(128, 256, kernel_size = 5, stride = 1, bias = True), 
                 #nn.BatchNorm1d(256),
                 #nn.MaxPool1d(2),
        ...
        #self.initial_layers = nn.ModuleList([
            #nn.Linear(self.strain_length, block_dims[0])
         #   conv_block
            #for _ in range(self.strain_channels)
        #])
        ...
        #self.linear_out = nn.ModuleList([nn.Sequential(nn.LazyLinear(dim), self.activation) for dim in block_dims])
        
    def forward(self, strain):
        '''
        out_CNN = torch.tensor([], device=strain.device)
        for i, cnn_layer in enumerate(self.CNN):
            #out_CNN.append( cnn_layer(strain[:, i, :].unsqueeze(1)) )
            out_CNN = torch.cat( [out_CNN, cnn_layer(strain[:, i, :].unsqueeze(1))]  , dim = -1 )
        '''
        ...
        for cnn_layer in self.CNN_localization:
            #out_CNN_localization.append( cnn_layer (strain))
        ...
        '''
        for layer in self.linear_out[:-1]:
            out = layer(out)
        out = self.Dropout(out)
        out = self.linear_out[-1](out)
        '''
